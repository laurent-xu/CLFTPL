...
class task_scheduler
{
  ...
  using task_mapper_t = std::unordered_map<KeyType, task_group>;
  ...
  private:
    ...
    static void run_group(task_mapper_t& tasks,
                          clfctpl::thread_pool<queue_t>& tp,
                          const KeyType& key)
    {
      auto& task_group = tasks[key];
      if (0 == task_group.second.jobs.size())
        release_targets(tasks, tp, key);
      else
        for (auto& job: task_group.second.jobs)
          tp.push(job);
    }
    ...
    clfctpl::thread_pool<queue_t> tp_;
